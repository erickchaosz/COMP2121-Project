...
;	inc nInterrupt
	...
	//	rcall lcd_wait
		...
		do_lcd_command 0b11000000	; line 2
...
;==================================
	...
	EXT_INT2: // avoid bounce
		cpi debounce, 0
		breq isDebounced2
		reti

		isDebounced2: // inc cycles without writing to sreg
			inc debounce
			push temp
			in temp, SREG
			push temp
...
// MAP INTERRUPTS FROM PINS
	ldi temp, (2 << ISC30) | (0<<ISC10) | (0<<ISC00)
	sts EICRA, temp

	in temp, EIMSK
	ori temp, (1<<INT0) | (1<<INT1) | (1<<INT2)			; enable INT2 in temp
...
// TIMER INTERRUPT AND MASK AND STUFF
	ldi temp, 0b00000010
	sts TCCR5B, temp
